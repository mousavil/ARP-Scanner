...
parser.add_argument('-s','--start',dest="start_ip_addr",type=str, metavar="START_IP_ADDR")
parser.add_argument('-e','--end',dest="end_ip_addr",type=str, metavar="END_IP_ADDR")
parser.add_argument('-r','--range',dest="ip_range", metavar="IP_RANGE", nargs="*")
...
if args.ip_range is not None:
    ...
    net_range = [args.start_ip_addr[:-3] + str(i) for i in range(int(args.start_ip_addr[-3:]), int(args.end_ip_addr[-3:]))]
else:
    print("Wrong Input")
    sys.exit()

for ip in  net_range:
    host_status=arp_request(interface=args.interface, dest_ip=ip, timeout=args.timeout)
